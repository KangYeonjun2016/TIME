sap.ui.define(
  ...
  (
    ...
    return BaseController.extend('sap.ui.time.mvc.controller.timeRegistration.Main', {
      ...
      _hasCompensationLeave: false, // 보상휴가 체크여부
      ...
      */
      ...
      async retrieveTimePersInfo() {
        try {
          ...
          mResult.Recal = mResult.Recal == 'X' ? '오류가 수정된 로직이 반영된 수치입니다.' : '';
      ...
      */
      ...
      async readAttachAppno() {
        try {
          ...
          oViewModel.setProperty('/form/dialog/data/FileAppno', aResults[0].Appnof && aResults[0].Appnof != '' ? aResults[0].Appnof : aResults[0].Appno);
        } catch (oError) {
          throw oError;
        }
      },
        ...
        */
        ...
        for (var i = 0; i < vMyDailyWorkTimeList.length; i++) {
          if (vMyDailyWorkTimeList[i].Beguz == vBeguz && vMyDailyWorkTimeList[i].Enduz == vEnduz) {
            ...
            oViewModel.setProperty(
              '/form/dialog/data/FileAppno',
              vMyDailyWorkTimeList[i].Appnof && vMyDailyWorkTimeList[i].Appnof != '' ? vMyDailyWorkTimeList[i].Appnof : vMyDailyWorkTimeList[i].Appno
            );
            oViewModel.setProperty(
              '/form/dialog/data/Susda',
      ...
      */
      ...
      async createPersAddCoreTime() {
        const oViewModel = this.getViewModel();
        const oModel = this.getModel(ServiceNames.MYTIME);
        const oMessageModel = this.getModel(ServiceNames.HRCALENDAR);
        ...
        const vTmdat = _.cloneDeep(formData.Tmdat),
        ...
        var rMessage = '';
        if (!this._hasCompensationLeave) {
          rMessage = await Client.getEntitySet(oMessageModel, 'OtApplyPopUp', {
            Pernr: this.getAppointeeProperty('Pernr'),
            Awart: formData.Awart,
            Zyymm: vTmdat.substring(0, 6),
          });

          if (rMessage.length > 0) {
            this._hasCompensationLeave = true;
            const aMessage = rMessage[0].Message.replace('\\n', '\n');
            MessageBox.alert(aMessage, {
              onClose: () => {
                this.createPersAddCoreTime2();
              },
            });
          } else {
            this.createPersAddCoreTime2();
      ...
      */
      SelectMyDailyWorkTime(oEvent) {
        ...
        if (oEvent.getSource().getSelected() == true) {
          ...
          oViewModel.setProperty('/form/dialog/data/FileAppno', vMyDailyWorkTime.Appnof);
      ...
      settingsAttachTable() {
        ...
        const sAppno =
          oViewModel.getProperty('/form/dialog/data/FileAppno') && oViewModel.getProperty('/form/dialog/data/FileAppno') != ''
            ? oViewModel.getProperty('/form/dialog/data/FileAppno')
            : oViewModel.getProperty('/form/dialog/data/Appno');
