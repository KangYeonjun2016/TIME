sap.ui.define(
  [
    // prettier 방지용 주석
    'sap/ui/time/control/ODataModel',
  ...
  (
    ...
    const ServiceManager = {
      ...
      // },
      getODataModel(sServiceName) {
        const sServiceUrl = this.getServiceUrl(sServiceName) + '?' + new Date().getTime(); //moment(new Date()).format('YYYYMMDD');
        return new ODataModel(sServiceUrl, { loadMetadataAsync: true, useBatch: false });
      },
      ...
      getMetadata(oServiceModel) {
        ...
        if (Array.isArray(oSchema.association)) {
          oSchema.association.forEach((p) => {
            let vType = _.find(p.end, { multiplicity: '*' });
            if (vType) {
